...
@Builder
public record NoticeImageCreateRequest(
        String imageUrl
) {
    // DTO를 FarmNoticeImage 엔티티로 변환하는 메서드
    public static FarmNoticeImage toEntity(FarmNotice farmNotice, String imageUrl) {
    ...
    public static List<FarmNoticeImage> toEntityList(FarmNotice farmNotice, List<String> imageUrls) {
        List<FarmNoticeImage> farmNoticeImages = new ArrayList<>();
        if (imageUrls != null) {
            for (String imageUrl : imageUrls) {
                farmNoticeImages.add(NoticeImageCreateRequest.toEntity(farmNotice, imageUrl));
            }
        }
